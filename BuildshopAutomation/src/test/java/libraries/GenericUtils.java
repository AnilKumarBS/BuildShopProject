...
public class GenericUtils {
	...
	*/
	...
	public static int getTableRowCountBySectionName(WebDriver driver, String sectionName){
		String xpth = "(//*[contains(text(),'"+sectionName+"')]/following-sibling::*/descendant::table)[1]/tbody/tr";
		List<WebElement> rows = driver.findElements(By.xpath(xpth));
		return rows.size();
	}
	...
	public static void WaitForAjax(WebDriver driver){
		try {
			...
			wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("div.pace.pace-inactive")));
		} catch (Exception e)
		{
	...
	public static double getCalculatedTotalEstimateInPortal(WebDriver driver){
		List<WebElement> prices = driver.findElements(By.xpath("//tr[@ng-class='item.item_entry' and not(descendant::td[@ng-bind='item.ext_price | currency' and contains(text(),'(')])]//td[@ng-bind='item.ext_price | currency']"));
		double sum=0.00;
		for(int i=0;i<prices.size();i++){
			String priceVal = prices.get(i).getText().replace("$", "");
	...
	public static double getTotalEstimateValueInPortal(WebDriver driver)
	{
		String TotalVal = driver.findElement(By.xpath("//table[contains(@class,'estimate-invoice-table')]/descendant::td[text()='Total']/following-sibling::td[@class='text-right ng-binding']")).getText().replace("$", "");
		double actualTotal = Double.parseDouble(TotalVal);
		return actualTotal;
	}	
}